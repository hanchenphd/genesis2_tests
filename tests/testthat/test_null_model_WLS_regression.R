...
expect_true(all(abs(nullmod$AIC - (lmm.genesis$AIC - 2)) < 1e-5))
...
expect_true(all(abs(nullmod$varCompCov - lmm.genesis$varCompCov[2:3, 2:3]) < 1e-4))  ## smaller than 1e-4, but not 1e-5, need to check!
